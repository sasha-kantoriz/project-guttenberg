"""
gutenberg_bundles_hardcover.py.

usage: python3 gutenberg_bundles_hardcover.py [options]

Project Gutenberg books scrape script (Hardcover Bundles):
...
logger = logging.getLogger("pg-bundles-hc")


def load_current_progress():
    try:
        with open('index_bundles_hardcover', 'r') as f:
...
def dump_current_progress(index):
    with open('index_bundles_hardcover', 'w') as f:
        f.write(str(index))
...
def main(folder, num_workers):
    ...
    try:
        wb = openpyxl.load_workbook('Project Gutenberg Bundles Hardcover.xlsx')
        ws = wb["Sheet"]
    except FileNotFoundError:
        ...
        for future in concurrent.futures.as_completed(future_to_row):
            ...
            if result:
                ...
                if processed_count % num_workers == 0:
                    ...
                    wb.save('Project Gutenberg Bundles Hardcover.xlsx')
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        prog='gutenberg-bundles-hardcover.py',
        usage='python3 %(prog)s [options]',
        description='Project Gutenberg books scrape script (Hardcover Bundles):',
